...
class SwinTransformer(nn.Module):
    ...
    def restore_gene(self, mask):
        ids_shuffle = torch.argsort(
            mask + (torch.arange(mask.shape[1]) / (mask.shape[1] * 5.)).unsqueeze(0).to(mask.device))
...
class ViT_Decoder(nn.Module):
    def __init__(self, inp_dim, embed_dim=256, out_dim=27, num_patches=196, depth=1, num_heads=8, mlp_ratio=4.,
    ...
    def forward(self, x, ids_restore):
        ...
        x = x.reshape(x.size(0), 7, 7, int(x.size(1) ** 0.5 / 7), int(x.size(1) ** 0.5 / 7), -1).transpose(2,
                                                                                                           3).reshape(
            x.shape)
...
class MaskedAutoencoderSwin(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
        ...
        self.encoder = SwinTransformer(img_size=img_size, patch_size=patch_size, in_chans=in_chans, num_classes=0,
        ...
        self.decoder = nn.ModuleList([
            ViT_Decoder(embed_dim * s, decoder_embed_dim // k,
                        in_chans * hog_nbins if target == 'HOG' else in_chans * (patch_size * s) ** 2,
