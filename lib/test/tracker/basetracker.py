...
class BaseTracker:
    ...
    def visdom_draw_heatmap(self, pred_score_map, segmentation=None):
        # if isinstance(box, OrderedDict):
        #     box = [v for k, v in box.items()]
        # else:
        #     box = (box,)
        if segmentation is None:
            self.visdom.register(pred_score_map, 'heatmap', 1, 'heatmap')
        else:
            self.visdom.register((pred_score_map, segmentation), 'heatmap', 1, 'heatmap')
