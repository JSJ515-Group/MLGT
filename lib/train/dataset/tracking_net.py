...
def list_sequences(root, set_ids):
    ...
    for s in set_ids:
        anno_dir = os.path.join(root, "TRAIN_" + str(s), "TRAIN_" + str(s), "anno")
...
class TrackingNet(BaseVideoDataset):
    ...
    def _read_bb_anno(self, seq_id):
        ...
        bb_anno_file = os.path.join(self.root, "TRAIN_" + str(set_id),  "TRAIN_" + str(set_id), "anno", vid_name + ".txt")
    ...
    def _get_frame(self, seq_id, frame_id):
        set_id = self.sequence_list[seq_id][0]
        vid_name = self.sequence_list[seq_id][1]
        frame_path = os.path.join(self.root, "TRAIN_" + str(set_id), "TRAIN_" + str(set_id), "frames", vid_name, str(frame_id) + ".jpg")
