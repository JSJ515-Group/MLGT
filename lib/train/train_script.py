...
from lib.train.actors import MLGTActor
...
def run(settings):
    ...
    # Create network
    if 'mlgt' in settings.script_name:
        net = build_mlgt(cfg)
    else:
    ...
    if 'mlgt' in settings.script_name:
        ...
        actor = MLGTActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError('ERROR: illegal script name')
